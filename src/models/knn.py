"""
Module implementing K Nearest Neighbors
"""
...
class KNEstimator():
    def __init__(self, params: dict):
        ...
        if self.pca is not None:
            self.name += "_pca"
            self.pca = params["pca"]
        model_params = {k:v for k,v in params.items() if k not in ["cv_folds","beta","scaler","seed","pca","pca_level","scaler_type"]}
        self.classifier = KNeighborsClassifier(**model_params)

    def cross_validate(self, X_train: np.array, y_train: np.array):
        splitter = StratifiedKFold(n_splits=self.cv_folds, shuffle=True, random_state=self.seed)
        score = 0.0
        self.cv_scores = {
            "train": {
                "precision": [],
