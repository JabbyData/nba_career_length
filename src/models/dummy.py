"""
Module implementing dummy classifier
"""
import numpy as np
...
class DummyEstimator():
    def __init__(self, params: dict):
        ...
        model_params = {k:v for k,v in params.items() if k not in ["cv_folds","beta","model","pca"]}
        self.classifier = DummyClassifier(**model_params)
        self.cv_scores = {
            "train": {
                "precision": [],
                "recall": [],
    ...
    def evaluate(self, X_test: np.array, y_test: np.array):
        ...
        fbeta = fbeta_score(y_test, dummy_preds, beta=self.beta, zero_division=0)
        return {
            "precision": precision,
            "recall": recall,
            f"F{self.beta}": fbeta,
