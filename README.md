...
| Modèle | Avantages | Inconvénients | Complexité temporelle (entraînement/inférence) | Complexité spatiale |
| :--- | :--- | :--- | :--- | :--- |
| [K-Nearest Neighbors](src/knn.ipynb) | 1. Entraînement rapide<br>2. Intuitif<br>3. Interprétable | 1. Sensible au bruit et à la dimension<br>2. Sensible à l'échelle des données<br>3. Temps d'inférence long | $O(1) / O(n_{test} \times n_{train} \times d)$ | $O(n_{train} \times d)$ |
| [Logistic Regression](src/logreg.ipynb) | 1. Aucune hypothèse sur les distributions d'entrée<br>2. Rapide<br>3. Interprétable | 1. Hypothèse de relation linéaire entre les caractéristiques et les log-odds<br>2. A du mal à capturer les relations complexes<br>3. Sensible à l'échelle des données et au bruit | $O(k_{iter} \times (n_{train} \times d^2 + d^3)) / O(n_{test} \times d)$ | $O(d)$ |
| [Support Vector Classifier](src/svc.ipynb) | 1. Très précis<br>2. Efficace dans les espaces de haute dimension<br>3. Polyvalent avec les noyaux<br>4. Robuste face aux valeurs aberrantes | 1. Sensible à l'échelle des données et au bruit (surtout features corrélées)<br>2. Temps d'inférence long quand $n \gg d$ | (polynomial) $O(n_{train}^2 \times d)$ à $O(n_{train}^3)$ / $O(k_{support} \times d^p)$ | $O(k_{support} \times d)$ |
| [Random Forest](src/random_forest.ipynb) | 1. Robuste face aux valeurs aberrantes<br>2. Gère les caractéristiques corrélées (méthode de sous-espace aléatoire)<br>3. Fonctionne avec des données de haute dimension<br>4. Importance des caractéristiques | 1. Risque de surajustement sur les petits ensembles de données<br>2. Complexité de calcul<br>3. Manque d'interprétabilité (sélection aléatoire) | $O(n_{arbres} \times \log{n_{train}}\times n_{train} \times d)$ / $O(n_{test} \times n_{arbres} \times \log{n_{train}})$ | $O(n_{arbres} \times \text{nombre de noeuds par arbre})$ |
